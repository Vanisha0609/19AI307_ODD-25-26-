...
## QUESTION:
A fuel station sells two types of fuels: Petrol and Diesel. There is a base class named Fuel, which has the following attributes: fuelId (a string representing the fuel transaction ID), customerName (the name of the customer), litersPurchased (the quantity of fuel bought in liters), and pricePerLiter (the cost of one liter of the fuel). Two classes, Petrol and Diesel, inherit from the Fuel class. The Petrol class applies a 3% discount on the total bill amount, whereas the Diesel class applies a 5% discount on the total bill amount. Your task is to write a program that accepts exactly three fuel purchase records. Each record starts with the fuel type (either Petrol or Diesel), followed by the fuelId, customerName, litersPurchased, and pricePerLiter. For each record, the program should calculate the total price after applying the respective discount and display the details of the purchase, including the discount rate and the final amount payable.
...
<img width="793" height="813" alt="image" src="https://github.com/user-attachments/assets/8a10161a-904c-404b-8b58-12b717cc150c" />
